...
export function UpdateMarkupButton({ onUpdated }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const markup_type = rate != null ? "percent" : null;
        const markup_value = rate;
        const { error } = await supabase
          .from("clients")
          .update({ markup_type, markup_value } as never)
          .eq("id", existingId);
        if (error) throw error;
        updated++;
      }

      toast.success(`עודכנו ${updated} לקוחות`);
      onUpdated?.();
    } catch (err) {
      console.error(err);
      toast.error("שגיאה בעדכון: " + (err instanceof Error ? err.message : "לא ידוע"));
    } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      >
        ...
        {loading ? "מעדכן..." : "עדכן אחוזי גבייה מאקסל"}
      </Button>
    </>
  );
}